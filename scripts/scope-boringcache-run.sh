...
grep -Fq 'tag = "immich-docker-local"' "$config_path" || { echo "Missing expected local Docker tag in ${config_path}" >&2; exit 1; }
sed -i "s/tag = \"immich-docker-local\"/tag = \"${scope}-docker\"/" "$config_path"
